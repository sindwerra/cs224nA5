...
Note: you will find `random.randint` useful shfor this part of the assignment.
...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        print(f"Complete Doc: {document}")

        # step 1
        truncated_length = random.randint(4, self.max_context_size)
        # random_start_index = random.randint(0, len(document) - truncated_length)
        # truncated_doc = document[random_start_index: min(len(document), random_start_index + truncated_length)]
        truncated_doc = document[:min(truncated_length, len(document))]

        print(f"Truncated Doc: {truncated_doc}")

        # step 2
        quartered_length = truncated_length / 4
        masked_content_length = int(random.uniform(quartered_length * 0.8, quartered_length * 1.2))
        mask_start_index = random.randint(1, truncated_length - masked_content_length - 1)
        prefix = truncated_doc[:mask_start_index]
        masked_content = truncated_doc[mask_start_index: mask_start_index + masked_content_length]
        suffix = truncated_doc[mask_start_index + masked_content_length:]
        print(prefix)
        ...
        # step 5
        x = [self.stoi[char] for char in input_string]
        y = [self.stoi[char] for char in output_string]
